...
echo "Debug: Checking nvidia-smi availability..."
...
if [ "${NVIDIA_VISIBLE_DEVICES}" != "all" ] && [ "${NVIDIA_VISIBLE_DEVICES}" != "none" ] && [ "${NVIDIA_VISIBLE_DEVICES}" != "void" ] && [ -n "${NVIDIA_VISIBLE_DEVICES}" ]; then
  echo "Debug: Selecting GPU from NVIDIA_VISIBLE_DEVICES"
  export GPU_SELECT="$(nvidia-smi --id=$(echo ${NVIDIA_VISIBLE_DEVICES} | cut -d ',' -f1) --query-gpu=uuid --format=csv,noheader 2>/dev/null | head -n1 | tr -d '[:space:]')"
  # If specific GPU selection failed, fall back to first available
  if [ -z "${GPU_SELECT}" ] || [[ "${GPU_SELECT}" == *"ERROR"* ]] || [[ "${GPU_SELECT}" == *"not recognized"* ]]; then
    echo "Warning: Failed to select GPU from NVIDIA_VISIBLE_DEVICES=${NVIDIA_VISIBLE_DEVICES}, using first available GPU"
    export GPU_SELECT="$(nvidia-smi --query-gpu=uuid --format=csv,noheader 2>/dev/null | head -n1 | tr -d '[:space:]')"
  fi
# Get first GPU device out of all visible devices in other situations
else
  echo "Debug: Selecting first available GPU"
  export GPU_SELECT="$(nvidia-smi --query-gpu=uuid --format=csv,noheader 2>/dev/null | head -n1 | tr -d '[:space:]')"
fi

echo "Debug: Selected GPU_SELECT=${GPU_SELECT}"
...
echo "Debug: Getting PCI Bus ID for GPU ${GPU_SELECT}"
HEX_ID="$(nvidia-smi --query-gpu=pci.bus_id --id=${GPU_SELECT} --format=csv,noheader 2>/dev/null | head -n1 | tr -d '[:space:]')"
echo "Debug: Raw HEX_ID from nvidia-smi: '$HEX_ID'"

# If nvidia-smi failed or returned empty result, try without GPU_SELECT
if [ -z "$HEX_ID" ] || [[ "$HEX_ID" == *"ERROR"* ]] || [[ "$HEX_ID" == *"not recognized"* ]]; then
  echo "Warning: Failed to get PCI Bus ID for GPU ${GPU_SELECT}, trying first available GPU"
  HEX_ID="$(nvidia-smi --query-gpu=pci.bus_id --format=csv,noheader 2>/dev/null | head -n1 | tr -d '[:space:]')"
  echo "Debug: Fallback HEX_ID: '$HEX_ID'"
fi

# Check if HEX_ID contains valid data
echo "Debug: Validating HEX_ID format: '$HEX_ID'"
if [[ "$HEX_ID" =~ ^[0-9A-Fa-f]{8}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}\.[0-9A-Fa-f]$ ]] || [[ "$HEX_ID" =~ ^[0-9A-Fa-f]{4}:[0-9A-Fa-f]{2}:[0-9A-Fa-f]{2}\.[0-9A-Fa-f]$ ]]; then
  echo "Debug: HEX_ID format is valid, parsing components"
  IFS=":" ARR_ID=(${HEX_ID})
  unset IFS
  echo "Debug: Parsed components: [0]='${ARR_ID[0]}' [1]='${ARR_ID[1]}' [2]='${ARR_ID[2]}'"
  # Validate each component before printf
  if [[ "${ARR_ID[0]}" =~ ^[0-9A-Fa-f]+$ ]] && [[ "${ARR_ID[1]}" =~ ^[0-9A-Fa-f]+$ ]] && [[ "${ARR_ID[2]%.*}" =~ ^[0-9A-Fa-f]+$ ]] && [[ "${ARR_ID[2]#*.}" =~ ^[0-9A-Fa-f]+$ ]]; then
    ARR_ID[2]="${ARR_ID[2]%.*}"
    ARR_ID[3]="${HEX_ID##*.}"
    echo "Debug: Final components for conversion: [0]='${ARR_ID[0]}' [1]='${ARR_ID[1]}' [2]='${ARR_ID[2]}' [3]='${ARR_ID[3]}'"
    BUS_ID="PCI:$(printf '%u' 0x${ARR_ID[1]})@$(printf '%u' 0x${ARR_ID[0]}):$(printf '%u' 0x${ARR_ID[2]}):$(printf '%u' 0x${ARR_ID[3]})"
    echo "Debug: Successfully converted to BUS_ID: '$BUS_ID'"
  else
    echo "Warning: Invalid PCI Bus ID format from nvidia-smi: $HEX_ID. Using fallback."
    BUS_ID="PCI:1@0:0:0"
  fi
else
  echo "Warning: nvidia-smi returned invalid or error data: $HEX_ID. Using fallback PCI Bus ID."
  BUS_ID="PCI:1@0:0:0"
fi
